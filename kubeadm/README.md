...
- `master.sh` runs `kubeadm init`, writes a join command to `generated/join-command.sh`, then installs [Flannel](https://github.com/flannel-io/flannel) as the cluster's CNI and [local-path-provisioner](https://github.com/rancher/local-path-provisioner) as the default StorageClass.
